...
`.github/workflows/build-otp-graph.yml` checks the upstream GTFS feed daily
and only rebuilds when it actually changed (or unconditionally on manual
`workflow_dispatch`), publishing the result to the `otp-graph` GitHub
Release — but CI has no access to the server, so nothing pulls that new
graph down on its own unless this is set up. Without it, the server's
`otp/graph.obj` only ever changes when someone manually repeats the steps
below — which is exactly how it silently went ~10 days stale before this was
added, quietly breaking any route whose schedule changed in that window
(right route, right geometry, just zero trips for "today" until someone
happened to reload the graph).
...
Runs daily (`otp-graph-sync.timer`) rather than trying to line up with the
build workflow's own schedule, so an off-schedule manual rebuild still
reaches the server within a day with no extra handling. Each run is a no-op
unless the
