...
export async function GET(request: Request) {
  ...
  if (!fromLat || !fromLng || !toLat || !toLng) {
    return NextResponse.json(
      { error: 'Invalid coordinates - make sure both addresses have been resolved with coordinates' },
      { status: 400 }
    )
  }
  ...
  try {
    console.log('Calling OTP at:', OTP_BASE_URL)
    
    const fullUrl = `${OTP_BASE_URL}/otp/gtfs/v1`
    ...
    if (!response.ok) {
      console.error(`OTP returned ${response.status}`)
      if (response.status === 404 || response.status === 503) {
        return NextResponse.json(
          { error: 'Route planning service is not available. Please ensure the OTP service is running at ' + OTP_BASE_URL },
          { status: 502 }
        )
      }
    ...
    if (data.errors?.length) { const nonFareErrors = data.errors.filter((e: any) => !e.message?.includes('currency')); if (nonFareErrors.length > 0 && itineraries.length === 0) { console.error('OTP GraphQL errors:', data.errors); return NextResponse.json({ error: nonFareErrors[0].message || 'Route planning failed' }, { status: 502 }); } }

    if (itineraries.length === 0) {
      return NextResponse.json({ error: 'No routes found between these locations' }, { status: 404 })
    }
    ...
    console.error('Failed to fetch route plan:', errorMsg)
    
    if (errorMsg.includes('ECONNREFUSED') || errorMsg.includes('timeout')) {
      return NextResponse.json(
        { 
          error: `Route planning service unavailable at ${OTP_BASE_URL}. Make sure the OTP service is running.`,
          details: 'ECONNREFUSED error - unable to connect to OTP service'
        },
        { status: 503 }
      )
    }
