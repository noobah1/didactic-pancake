...
export async function GET(request: Request) {
  ...
  try {
    ...
    // For now, use mock alerts in development
    // In production, replace with real OTP service URL
    if (OTP_BASE_URL === 'http://localhost:3000' || OTP_BASE_URL.includes('localhost')) {
      // Use mock alerts (already defined above)
      cache = { data: MOCK_ALERTS, timestamp: now }
      return NextResponse.json({ alerts: MOCK_ALERTS, timestamp: now })
    }

    // Use real OTP service for alerts
